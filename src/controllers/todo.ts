...
export const createTodo: RequestHandler = (req, res, next) => {
    ...
    TODOS.push(newTodo);
    res.status(200).json({message: 'created thetodo', createdToDo: newTodo})
}
...
export const delTodos: RequestHandler = ( req, res, next) => {
    const todoId = req.params.id;
    ...
    res.status(200).json({message: 'Delete', todoId})
}